...
We have also considered (log_taxpk, pctymale) for this model, but we concluded that none of these is stasitcally signficant.
...
In this model, we'll include the variables (avgsen, mix), as we think they will contribute to the accuracy of your results without introducing substantial bias. These variables show good degree of correlation with crime rate, as well as, linear relationship as observed in the EDA. 
...
pctmin80 was also considered for this model. Although it is statistically signfifent, we realized that a negative correlation between western counties and pctmin80.

#TODO - Gaurav to add more from his head analysis! i.e why central.


$$crimeDeterm = \beta_0 + \beta_1 \cdot density + \beta_2 \cdot prbarr + \beta_3 \cdot prbconv + \beta_4 \cdot west\\
 + \beta_5 \cdot avgsen + \beta_6 \cot mix + \beta_7 \cdot central$$

```{r  fig.align='center', fig.height=3, fig.width=3, fig.show='hold'}
model2 <- lm(log_crmrtepk ~ density + prbarr + prbconv + west 
             + avgsen + mix + central, data=crimeData)
summary(model2)$coefficients[,"Estimate"]
summary(model2)$cov.unscaled
summary(model2)$adj.r.squared
```

We see slight increase in model fit. We are able to fit 70% of our population by adding 3 new covariates into model 1.
But at the same time we see that some of the new covariates added in model 2 have high correlation with existing variables. This will induce multicollinearity into our model.
...
$$crimeDeterm = \beta_0 + \beta_1 \cdot density + \beta_2 \cdot prbarr + \beta_3 \cdot prbconv + \beta_4 \cdot west\\
 + \beta_5 \cdot avgsen + \beta_6 \cdot mix + + \beta_7 \cdot central + \\
\beta_8 \cdot pctmin80 + \beta_9 \cdot log_taxpk + \beta_{10} \cdot urban + \beta_{11} \cdot pctmyle\\
\beta_{12} \cdot prbpris + \beta_{13} \cdot log_polpk + \beta_{14} \cdot wtotal$$

```{r  fig.align='center', fig.height=3, fig.width=3, fig.show='hold'}
model3 <- lm(log_crmrtepk ~ density + prbarr + prbconv + west
             + avgsen + mix + central + pctmin80 + log_taxpk + urban 
             + pctymle + prbpris  + log_polpk + wtotal, data=crimeData)
summary(model3)
...
stargazer(model1, model2, model3
          ...
          ,covariate.labels = c("Probability of Arrest", "Probability of Conviction"
                                ...
                                , "Is Central County","% of Minority"
                                , "Log of tax per K", "Is Urban County", "% of Male"
...
The above summary deomonstrates the following:
...
4. Adding additional variables to model 2 could have increasd the R squared but we'll introduce violations to CLM assumptions.

5. Although Model 3 has the highest R square, CLM assumptions such as linearity and multicollinearity.

## CLM Assumptions Analysis

In this section we'll analyze CLM assumptions of Model 1.
...
2. The residual vs leverage indicates that the outlier (#51 and #53 to be precise). Observation #51 in particular, crosses Cook's distance, which is concerning. We analyzed what it is special about it and realized it has outliers values for all three variables. Our recommendation would be to remove this observation, which will demonstrate that the assumption is not violated.
...
We don't see any of the assumptions being violated for model 2.

### Model3 CLM Assumptions Analysis

We see 2 assumption Multicollinearity and Homoskedasticity showing alarming results. Lets see them in detail.
#### Assumption.4 - Multicollinearity
